...
from contest.data import event_log
from contest.models.vae import LSTMVAE
from contest.data import event_log
from contest.models.blackbox import (
...
from contest.search.counterfactual import LocalSurrogate, CounterfactualSearch
from contest.utils.training import (
...
class Pipeline:
    ...
    def fit(
        ...
        # 1. Dynamically extract the true feature dimension from the actual data
    ...
    def explain(
        ...
        from contest.models.blackbox import feature_names
        ...
        def _build_rigid_invariant_decoder(
            ...
            editable_row_indices: Optional[Set[int]] = None,
        ):
            ...
            def rigid_invariant_decoder(z_tensor):
                ...
                return raw_seq  # Output retains 3D Tensor context: (B, step, event_dim)
        ...
        from contest.models.blackbox import feasible_joint_action_indices
        ...
        if verbose:
            # logger.info(f"Surrogate trained | final loss={surr_history[-1]:.4f}")
            with torch.no_grad():
        ...
        while (
            ...
            _attempt_record = {
                ...
                "cf_action_idx": int(cf_result.cf_action_idx),
                "cf_act": cf_act,     # from idx_to_joint_action(cf_result.cf_action_idx)
                "cf_res": cf_res,
    ...
    def load(self, path: str = "latent_space_weights") -> "Pipeline":
