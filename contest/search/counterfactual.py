...
class LocalSurrogate(nn.Module):
    ...
    def predict(self, z: torch.Tensor) -> torch.Tensor:
        ...
        from contest.models.blackbox import non_recommendable_action_indices
    ...
    def search(
        ...
        for it in range(self.max_iter):
            ...
            if is_genuinely_stuck:
                ...
                loss = prox + self.lambda1 * l_man + self.lambda2 * l_spar + self.lambda3 * l_class + 2.0 * expulsion_penalty + 5.0 * l_repulsion
            else:
                loss = prox + self.lambda1 * l_man + self.lambda2 * l_spar + self.lambda3 * l_class + 5.0 * l_repulsion
            ...
            with torch.no_grad():
                ...
                if target_action_idx is not None:
                    if real_bb_pred == target_action_idx:
                        best_z = z_cf.detach().clone()
                        exit_reason = "flipped"
                        break
                else:
                    if real_bb_pred != original_action_idx:
                        best_z = z_cf.detach().clone()
                        exit_reason = "flipped"
                        break
