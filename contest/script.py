"""
Main script for running the CONTEST framework end-to-end
"""
...
from contest.data.event_log_csv import load_event_log_csv
from contest.data import event_log as el
from contest.pipeline import Pipeline
from contest.data import domain_persistence
...
def milestone_slice_idx(n: int, milestone: float) -> int:
    ...
    The floor of 1 guarantees at least one event of history to encode; the
    ceiling of n-1 guarantees at least one event remains "in the future" so
    the recommendation being explained corresponds to a real next step, not
    one past the end of the trace. For very short traces (n <= 2) both
    bounds can coincide, which is expected and not an error.
    """
    if n < 2:
        raise ValueError(
...
def compute_length_buckets(case_lengths, n_buckets: int = 4, method: str = "quantile"):
    ...
    Returns (edges, labels). edges has len(labels) + 1 entries. Raises if
    edges collapse (e.g. too many duplicate lengths for n_buckets).
    """
...
def parse_args() -> argparse.Namespace:
    ...
    # Milestone / contrast configuration (the parameters this script exists for)
    ...
    p.add_argument("--gamma-conf", type=float, default=1.5)
...
def main() -> None:
    ...
    # ── 1. Data — train and test are two separate, user-supplied CSVs ──────
    ...
    if weights_ok and not args.retrain:
        ...
        if args.use_conformance:
            from contest.data.event_log import build_activity_transition_matrix, ACTIVITIES, pm_row_entropy_floor
